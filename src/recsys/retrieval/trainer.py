...
class RetrievalDataset(Dataset):
    def __init__(
        ...
        for row in interactions.sort_values("timestamp").itertuples():
            ...
            if hist:
                self.samples.append((hist, item_idx))
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        hist, pos_item = self.samples[idx]
...
def _build_mappings(items: pd.DataFrame) -> tuple[dict[str, int], np.ndarray]:
    ...
    for row in items.itertuples():
        ...
        cat_code = hash(row.category) % 1000 / 1000.0
        features.append([price / 500.0, cat_code, 0.0, 0.0] + [0.0] * 12)
    return item_id_to_idx, np.array(features, dtype=np.float32)
...
def train_retrieval_model(
    ...
    item_id_to_idx, item_features = _build_mappings(items_df)
    ...
    train_ds = RetrievalDataset(train_df, item_id_to_idx, item_features, cfg["model"]["history_length"])
    ...
    with tracker.run("train_retrieval"):
        ...
        for epoch in range(num_epochs):
            model.train()
            total_loss = 0.0
            for histories, pos_items, pos_feats in train_loader:
                ...
                loss = logq_corrected_loss(logits, batch_pop, temperature=cfg["training"]["temperature"])
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
    ...
    all_feats = torch.tensor(item_features[all_indices.cpu().numpy() - 1], dtype=torch.float32, device=device)
    with torch.no_grad():
    ...
    val_samples = RetrievalDataset(val_df, item_id_to_idx, item_features, cfg["model"]["history_length"])
    if len(val_samples) > 0:
        histories, _, _ = val_samples[0]
